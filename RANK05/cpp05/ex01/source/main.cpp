#include "Form.hpp"

int main(void) {
    std::cout << MAGENTA << "========= Test Form =========" << RESET << std::endl;

    std::cout << BLUE << "\nTest 1: Invalid Form (sign=0, exec=10)" << RESET << std::endl;
    try {
        Form invalid("InvalidForm", false, 0, 10);
    } catch (const std::exception &e) {
        std::cerr << RED << "Exception caught: " << e.what() << RESET << std::endl;
    }

    std::cout << BLUE << "\nTest 2: Invalid Form (sign=200, exec=10)" << RESET << std::endl;
    try {
        Form invalid("InvalidForm", false, 200, 10);
    } catch (const std::exception &e) {
        std::cerr << RED << "Exception caught: " << e.what() << RESET << std::endl;
    }

    std::cout << BLUE << "\nTest 3: Valid Forms" << RESET << std::endl;
    Form tax("TaxForm", false, 100, 80);
    Form secret("SecretForm", false, 40, 20);

    std::cout << tax << std::endl;
    std::cout << secret << std::endl;

    std::cout << BLUE << "\nTest 4: Bureaucrats" << RESET << std::endl;
    Bureaucrat bob("Bob", 50);
    Bureaucrat tom("Tom", 120);

    std::cout << bob << std::endl;
    std::cout << tom << std::endl;

    std::cout << BLUE << "\nTest 5: Signing Forms" << RESET << std::endl;
    bob.signForm(tax);    // Bob grade 50 <= 100, should sign
    tom.signForm(tax);    // Tom grade 120 > 100, fail
    bob.signForm(secret); // Bob grade 50 > 40, fail
    tom.signForm(secret); // Tom grade 120 > 40, fail

    std::cout << tax << std::endl;
    std::cout << secret << std::endl;

    std::cout << BLUE << "\nTest 6: Copy & Assignment" << RESET << std::endl;
    Form fcopy = tax;  // copy constructor
    std::cout << fcopy << std::endl;

    Form fassign("Dummy", false, 150, 150);
    fassign = secret;  // assignment
    std::cout << fassign << std::endl;

    return 0;
}
